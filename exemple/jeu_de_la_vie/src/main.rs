...
pub struct App {
    ...
    matrix_etat: Vec<Vec<bool>>,
    color_sender: mpsc::Sender<(Vec<Vec<bool>>)>,
    color_receiver: mpsc::Receiver<(Vec<Vec<bool>>)>,
}
...
impl App {
    ...
    fn render(&mut self, args: &RenderArgs) {
        ...
        let alive = [1.0, 0.0, 0.0, 1.0];//red
        let dead = [1.0, 1.0, 0.0, 1.0];//jaune
       ...
       if draw_grid {
         ...
         //        .transform
         //        .trans(i as f64 * 32.0 , j as f64 * 32.0);
         // rectangle(colors_marge(7), square, transform, gl);
...
struct window_app {
  ...
  signal: Arc<Signal>,
  color_sender: Mutex<mpsc::Sender<(Vec<Vec<bool>>)>>,
  signal_1: Arc<Signal>,
  matrix_before_update: Arc<Mutex<Vec<Vec<bool>>>>,
  matrix_after_update: Arc<Mutex<Vec<Vec<bool>>>>,
...
impl Thread for window_app{
  ...
  impl window_app {
    // add code here
    fn new(scheduler: Arc<Scheduler>, signal: Arc<Signal>, signal_1: Arc<Signal>, m1: Arc<Mutex<Vec<Vec<bool>>>>, m2: Arc<Mutex<Vec<Vec<bool>>>> ,color_sender: mpsc::Sender<(Vec<Vec<bool>>)>)-> window_app{
...
fn main(){
  ...
  println!("Hello");
//   let mut x: Arc<Mutex<Vec<Vec<bool>>>> = Arc::new(Mutex::new(vec![vec![false; 20]; 20]));
...
// x.lock().unwrap().clear();
//   println!("x {:?}", y);
//   let tuple = get_neighbours(19,19, 20, 20);
//   println!("{:?}",tuple );
//   for i in tuple.iter(){
//   	let (a, b ) = i;
//   	println!("iiii {:?}\n", a );
...
//   /********************** Piston *****************/
   // Change this to OpenGL::V2_1 if not working.
    ...
    let mut app = App {
        ...
        rotation: 0.0,
        matrix_etat: vec![vec![false; 20]; 20],
   ...
   let mut events = Events::new(EventSettings::new());
            ...
            /**** Scheduler sche ****/
    let mut matrix_1: Arc<Mutex<Vec<Vec<bool>>>> = Arc::new(Mutex::new(vec![vec![false; 20]; 20]));    
    let mut matrix_2: Arc<Mutex<Vec<Vec<bool>>>> = Arc::new(Mutex::new(vec![vec![false; 20]; 20]));  
    ...
    for i in 0..20{
    	for j in 0..20{
    		let cell = Cell::new(Arc::clone(&arc_sche), Arc::clone(&signal), Arc::clone(&signal_1), matrix_1.clone(), matrix_2.clone(),  i as f64, j as f64);
