...
extern crate test_librairy;
use test_librairy::scheduler;
use test_librairy::thread_trait;
use test_librairy::signal;



use test_librairy::thread_trait::Thread_trait::Thread;
use test_librairy::scheduler::scheduler::Scheduler;
use test_librairy::signal::signal::Signal;


use test_librairy::thread_trait::Thread_trait::GLOBAL_THREAD_COUNT;
use test_librairy::scheduler::scheduler::GLOBAL_SCHEDULER_COUNT;
use test_librairy::scheduler::scheduler::My_scheduler;
use test_librairy::signal::signal::GLOBAL_SIGNAL_COUNT;
use test_librairy::thread_trait::Thread_trait::get_condvar;
...
impl Thread for Reactif_Thread2{
  ...
  fn return_nb_thread(&self) -> isize{
   ...
   fn thread_execute(&mut self){
    ...
    self.thread_cooperate();
    self.thread_await(self.signal2.clone(), Reactif_Thread2::is_here, Reactif_Thread2::is_not_here);
    println!("exit");
    process::exit(0x0100);
  }
}
...
impl Thread for Reactif_Thread3{
  ...
  fn return_nb_thread(&self) -> isize{
   ...
   fn thread_execute(&mut self){
    ...
    self.thread_cooperate_n(3);
    println!("signal1 generated");
    self.thread_emit(self.signal1.clone());

    self.thread_cooperate_n(10);
    println!("signal2 generated");
...
impl Reactif_Thread3 {
    pub fn new(sche: Arc<Scheduler>, s1:  Arc<Signal>, s2:  Arc<Signal>) -> Reactif_Thread3 {
         let old_thread_count = GLOBAL_THREAD_COUNT.fetch_add(1, Ordering::SeqCst);
         Reactif_Thread3{
          id_thread: old_thread_count+1,
            scheduler: sche,
...
fn main(){
    ...
    let th3 = Reactif_Thread3::new(Arc::clone(&arc_sche), Arc::clone(&signal), Arc::clone(&signal_1));
    th1.thread_run();
    th2.thread_run();
    th3.thread_run();
